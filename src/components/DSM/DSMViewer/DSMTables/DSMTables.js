...
class DSMTables extends Component {
  state = {
    gridApi: null,
    gridParams: null,
  ...
  createColumnDefsWithOptions = () => {
    const { rowData, isCmaTable, isReportsCmaTable, hiddenColumns, columns } = this.props;
            ...
            filterParams: {
              valueGetter: function getter(params) {
                ...
                if (isFormatFilter && formatFilterKeys.length && typeof val === 'object') {
                  formattedValue = dataTableComponent.customFormatObject(val, formatFilterKeys);
